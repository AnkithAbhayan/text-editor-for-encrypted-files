...
def initialize():
    ...
    if "anontrue" in sys.argv:
        return ankith.cryptography.decrypt(data)
    elif "anontrue" not in sys.argv and "anonfalse" not in sys.argv:
        if path[-11:] == ".ankiecrypt":
            sys.argv.append("anontrue")
            return ankith.cryptography.decrypt(data)
        else:
            sys.argv.append("anonfalse")
            return data
    else:  
        return data
